...
concept DynamicalSystem =
    ...
    requires(
        Sys sys,
        typename Sys::ScalarType t,
        typename Sys::ScalarType* x,
        typename Sys::ScalarType* u,
        typename Sys::ScalarType* dxdt)
{
...
// NOTE(amg): RK2 can allocate it's own memory as needed, but in fact in this case, does not even need to be a functor at all
class RK2 {
    public:

    template <DynamicalSystem Sys>
    __host__ __device__ void operator()(Sys& sys, 
        typename Sys::ScalarType dt, typename Sys::ScalarType t, 
        const typename Sys::ScalarType* x, const typename Sys::ScalarType* u, typename Sys::ScalarType* x_next) {
...
__global__ void rollout_kernel(Sys sys, Integrator integrator,
    const TensorView<const float, 1> x0, const TensorView<const float, 3> u_seq, float dt, TensorView<float, 3> x_seq) {
    ...
    for (int i = 0; i < T; ++i) {
        t += dt;
        integrator(sys, dt, t, x_seq.slice_1d<2>(batch_idx, i), u_seq.slice_1d<2>(batch_idx, i), x_seq.slice_1d<2>(batch_idx, i+1));
    }
}

template<DynamicalSystem Sys, typename Integrator> 
Tensor< typename Sys::ScalarType, 3> rollout_gpu(const Sys& sys, const Integrator& integrator, const Tensor<float, 1> &x0, const Tensor<float, 3> &u_seq, float dt) {
    int batch_size = u_seq.shape(0);
    int T = u_seq.shape(1);
    int n_x = sys.get_n_x();
    Tensor<float, 3> x_seq(batch_size, T+1, n_x);
    rollout_kernel<<<(batch_size + 255) / 256, 256>>>(sys, integrator, x0.view(), u_seq.view(), dt, x_seq.view());
    return x_seq;
}
...
__global__ void cost_kernel(
    Cost cost,
    TensorView<float, 3> x_seq,   // [N, T+1, n_x]
...
void cem(
    const Sys&         sys,
    const Integ&       integrator,
    const Tensor<float, 1>& x0,
    ...
    float              sigma_init = 0.5f)
{
    ...
    for (int iter = 0; iter < n_iters; ++iter) {
        ...
        for (int n = 0; n < n_samples; ++n)
            for (int t = 0; t < T; ++t)
                for (int j = 0; j < n_u; ++j)
                    samples(n, t, j) = mean[t * n_u + j]
                                     + sigma[t * n_u + j] * gauss(rng);
        ...
        std::partial_sort(indices.begin(), indices.begin() + n_elites, indices.end(),
