...
// By visual inspection this all looks correct to me, but more testing is
// probably warranted Tensor is intentionally using the unified memory right
// now. If this is a bottleneck we should be able to switch to manually managed
// memory fairly easily later. The TensorView really does not care. You can also
// make a TensorView from any raw pointer, not just a Tensor. It is somewhat
// unclear to me if we actually want to allow implcit conversion of TensorView
// to T* Nice style-wise, but sort of a footgun
...
struct TensorView
{
  ...
  template <typename... Idx, typename = std::enable_if_t<sizeof...(Idx) == dim>>
  ...
  {
    static_assert(
        dim > 1, "Cannot slice a 1-D view — use operator()(i) instead.");
    static_assert(fixed_dim >= 0 && fixed_dim < dim, "fixed_dim out of range.");
  ...
  // ------------------------------------------------------------------
  template <
      int kept_dim, typename... Idx,
      typename = std::enable_if_t<sizeof...(Idx) == dim - 1>>
  __host__ __device__ TensorView<T, 1> slice_1d(Idx... fixed_indices) const
  {
    static_assert(dim > 1, "slice_1d requires at least a 2-D view.");
    static_assert(kept_dim >= 0 && kept_dim < dim, "kept_dim out of range.");
...
class Tensor : public TensorView<T, dim>
{
 ...
 public:
  ...
  Tensor<T, dim> clone() const
  {
    ...
    // Only valid for tensors whose strides match a fresh row-major
    // allocation (i.e. not the result of a non-contiguous slice).
    // A Tensor always owns its own contiguous buffer, so this holds.
    CUDA_CHECK(cudaMemcpy(
        out.data(), this->data_, this->numel() * sizeof(T),
        cudaMemcpyDefault));  // works for managed memory, CPU<->GPU agnostic
...
template <typename T>
using TensorView1D = TensorView<T, 1>;
template <typename T>
using TensorView2D = TensorView<T, 2>;
template <typename T>
using TensorView3D = TensorView<T, 3>;